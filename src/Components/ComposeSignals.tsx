...
const ComposeSignals = ({ messages, recipients, signals }: Props) => {
  ...
  const signalsList: SignalsList = useSelector(
    (state: any) => state.manageSignals.signalsList
  );
  ...
  function handleRecipients(
    ...
    if (e.target.checked) {
      setRecipientList([...recipientList, e.target.value]);
    } else {
      setRecipientList((recipientList) =>
        recipientList.filter((item) => item !== e.target.value)
      );
    }
  ...
  async function postData() {
    if (readyState) {
      try {
        ...
        await setDoc(
          doc(db, "signalsList", signalsList.id),
          {
            id: signalsList.id,
            uid: uid,
          ...
          toast.success("Successfully submitted to firestore");
        });
        dispatch(resetForm());
          ...
          <FormControl fullWidth>
            ...
            >
              {messages?.map((item: CustomText) => (
                <MenuItem key={item.id} id={item.id} value={item.message}>
                  {item.message}
                </MenuItem>
              ))}
            </Select>
